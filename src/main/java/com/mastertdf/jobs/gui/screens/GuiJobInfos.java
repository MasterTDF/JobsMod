...
import com.mastertdf.jobs.gui.buttons.ButtonXPCategory;
import com.mastertdf.jobs.gui.buttons.SlideBarUnlock;
...
public class GuiJobInfos extends Screen {
    ...
    public void init() {
        ...
        this.addButton(new ButtonXPCategory(this.width / 2 - 84 + offsetUnlock, this.height / 2 + 40, XPCategories.XP, this));
        this.addButton(new ButtonXPCategory(this.width / 2 + 4 + offsetUnlock, this.height / 2 + 40, XPCategories.UNLOCK, this));
    ...
    private void drawUnlockedItems(MatrixStack mStack, int mouseX, int mouseY) {
        ...
        if (renderIndex != -1)
            renderComponentTooltip(mStack, getItemToolTip(new ItemStack(unlocked_items.get(renderIndex + page))), mouseX, mouseY);
